package com.buxz.online_exam.socket;

import com.buxz.online_exam.base.BaseContext;
import com.buxz.online_exam.entity.BusinessMain;
import com.buxz.online_exam.entity.UserInfo;
import com.buxz.online_exam.thread.Timekeeping;
...
public class MySocket2 {
    ...
    public void onOpen(Session session,EndpointConfig config) throws IOException{
        ...
        String 用户名 = (String)userInfoMap.get(UserInfo.username.toString());
    ...
    public void onClose(){
    	...
    	//3、
    	try {
    		context.removeAttribute((String)httpSession.getAttribute(UserInfo.username.toString()));
		} catch (Exception e) {
    ...
    public void onMessage(String message, Session session) {
        ...
        if (message != null){
			...
			case "start":
				...
				//计算答题时间，单位为：分钟
				Long targetMinute = 1000L*60L*Long.parseLong(time);
				...
				thread.start();
				break;
			//用户关闭了浏览器，会向服务器发送一条数据，这里可做持久化算分处理	
			case "close":
